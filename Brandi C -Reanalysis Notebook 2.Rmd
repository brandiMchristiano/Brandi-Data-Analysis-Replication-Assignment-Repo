...
# didn't use this for the analysis I did
...
```{r}
# To find the confidence intervals of the mean of these events, they boot strapped 10000 samples
boot_ci <- function(x, perms=10000) { # this is creating a basic function for boot_ci. using the data set (in this case, the fake data set"x"), it will run 10,000 permutations
  mean.w=function(x,w) sum(x*w)
  numNA <- sum(is.na(x)) # this counts how many NAs there are 
  x <- as.vector(na.omit(x))# this takes the data set and just makes it so there are no NAs in it
  mean <- mean(x) 
...
# trying to play with this on my own
...
boot_ci(time.to.share)

plot1 <- (boxplot((time.to.share), main= "Time Until First New Sharing"))
```
